...
router.post('/categories', authenticateAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO categories (name, image_url, cloudinary_public_id, description, sort_order)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [name.trim(), imageUrl, publicId, description || '', parseInt(sort_order) || 0]
    );
...
router.put('/categories/:id', authenticateAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    let publicId = existing.rows[0].cloudinary_public_id;
    ...
    const result = await pool.query(
      `UPDATE categories SET name=$1, image_url=$2, cloudinary_public_id=$3, description=$4, sort_order=$5
       WHERE id=$6 RETURNING *`,
      [name.trim(), imageUrl, publicId, description || '', parseInt(sort_order) || existing.rows[0].sort_order, id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    if (err.code === '23505') {
